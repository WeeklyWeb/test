...
//# sourceMappingURL=swiper-bundle.js.map;
 ...
 $(document).ready(function() {
    $('#order-form').submit(function(e) {
        ...
        var phone = $('#phone').val();
        var pattern = /^[0-9]+$/;
        var RegExp = pattern.test(phone);
        if (!RegExp) {
            $(".clue-phone").addClass("clue-view");
            $('.clue-phone').text('Допустимы только числа!');
        } else {
            $(".clue-phone").removeClass("clue-view");
    ...
    JQClass.classes.JQPlugin = JQClass.extend({
        ...
        _init: function() {
            ...
            $[a] = this, $.fn[a] = function(b) {
                var c = Array.prototype.slice.call(arguments, 1),
                    d = this,
                    e = this;
                return this.each(function() { if ("string" == typeof b) { if ("_" === b[0] || !$[a][b]) throw "Unknown method: " + b; var f = $[a][b].apply($[a], [this].concat(c)); if (f !== d && void 0 !== f) return e = f, !1 } else $[a]._attach(this, b) }), e
            }
        },
        ...
        _getMetadata: function(elem) {
            try {
                ...
                data = data.replace(/(\\?)'/g, function(a, b) { return b ? "'" : '"' }).replace(/([a-zA-Z0-9]+):/g, function(a, b, c) { var d = data.substring(0, c).match(/"/g); return d && d.length % 2 !== 0 ? b + ":" : '"' + b + '":' }).replace(/\\:/g, ":"), data = $.parseJSON("{" + data + "}");
                for (var key in data)
...
$('#time').countdown({ until: +1800, compact: true, format: 'MS' });;